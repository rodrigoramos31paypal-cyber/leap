-- ════════════════════════════════════════════════════════════════
-- 0125 · create_booking_admin · permite MESMO horário de início
--
-- A 0112 permitiu sobreposições parciais mas manteve o bloqueio quando
-- já existia uma marcação a começar EXACTAMENTE à mesma hora. Staff e
-- admins passam agora a poder marcar sessões mesmo nesse caso (ex.: dois
-- clientes em simultâneo no mesmo horário).
--
-- Igual à 0115 em tudo o resto (relaxações 0107/0112 + pool DUO 0113).
-- Só remove o check de `starts_at = p_starts_at`. Caminho do cliente
-- (`create_booking`) fica inalterado.
--
-- REVERT: reaplicar 0115.
-- ════════════════════════════════════════════════════════════════
...
begin
  ...
  -- 0125: sem bloqueio de horário de início repetido. Staff/admin pode
  -- marcar sessões em simultâneo (mesmo starts_at). Sobreposições
  -- parciais já eram permitidas desde a 0112.
