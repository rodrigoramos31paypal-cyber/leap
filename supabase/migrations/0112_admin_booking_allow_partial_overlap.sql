...
--   (tstzrange && tstzrange).
-- Agora: bloqueia apenas quando já existe uma marcação ativa com
--   `starts_at = p_starts_at`. Sobreposições parciais são permitidas.
--
-- Apenas o caminho ADMIN é afetado. O cliente (`create_booking`)
-- mantém a regra estrita — clientes não podem sobrepor.
--
-- Tudo o resto (passado→confirmed, DUO, descontos, notificações) é
-- idêntico à 0107.
--
-- REVERT: reaplicar 0107.
-- ════════════════════════════════════════════════════════════════
...
begin
  ...
  -- 0112: só bloqueia o MESMO horário de início exato. Sobreposições
  ...
  if exists (
    ...
    raise exception 'Já existe uma marcação que começa exatamente a esta hora.';
  end if;
