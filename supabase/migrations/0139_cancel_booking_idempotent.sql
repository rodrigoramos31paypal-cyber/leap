-- ════════════════════════════════════════════════════════════════
-- 0139 · cancel_booking IDEMPOTENTE (void → boolean)
--
-- Problema: cancel_booking (0132) devolvia void e fazia early-return
-- silencioso quando a sessão já estava cancelada/no_show. O caller TS
-- (cancelBookingAction / cancelAdminAction) enviava SEMPRE o email de
-- cancelamento + gravava auditoria a seguir, mesmo quando a RPC não
-- transitou nada. Resultado: em duplo/triplo-clique no botão "Cancelar",
-- a sessão era cancelada 1x mas saíam N emails "Sessão cancelada" (e N
-- linhas de auditoria) no mesmo minuto.
--
-- Correção: cancel_booking passa a devolver boolean —
--   true  = ESTE pedido transitou a sessão (activa → cancelada)
--   false = no-op (já estava cancelada/no_show)
-- O caller só envia email/audit/calendário quando true. Race-safe: o
-- `select ... for update` no topo serializa cancelamentos concorrentes
-- da mesma sessão, por isso só o 1º pedido vê o estado activo.
--
-- Corpo idêntico à 0132 (mesma autorização, reembolso, notificações);
-- só muda a assinatura e os dois pontos de saída.
--
-- REVERT: repor cancel_booking da 0132 (returns void).
-- ════════════════════════════════════════════════════════════════
...
begin
  ...
  -- IDEMPOTÊNCIA (0139): já cancelada/no_show → no-op. Devolve false
  -- para o caller NÃO reenviar email/auditoria (causa dos emails
  -- em triplicado no duplo-clique).
  if v_booking.status in ('cancelled', 'no_show') then return false; end if;
...
comment on function cancel_booking(uuid, text) is
  '0139: cancela uma marcação e devolve true só quando ESTE pedido a transitou (activa → cancelada); false se já estava cancelada/no_show. O caller usa isto para não duplicar email/auditoria em cliques repetidos.';
