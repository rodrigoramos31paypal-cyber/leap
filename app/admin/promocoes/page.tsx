...
import { Images, Plus, Pencil, Upload } from "lucide-react";
...
export default async function AdminSlideshowPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-tight">Slideshow</h1>
        <p className="text-sm text-ink-500">
          Até {MAX_SLIDES} slides com imagem e texto, mostrados no carrossel do dashboard
          dos clientes. {list.length}/{MAX_SLIDES} usados.
        </p>
      </div>
            ...
            <li key={b.id} className="card overflow-hidden">
              <div className="flex items-stretch gap-3 p-4">
                {b.image_url ? (
                  ...
                  <img src={b.image_url} alt={b.title} className="h-16 w-24 shrink-0 rounded-lg object-cover" />
                ) : (
                  <div className="grid h-16 w-24 shrink-0 place-items-center rounded-lg bg-bone-100 text-ink-400 dark:bg-white/5">
                    <Images size={18} />
                  </div>
                )}
