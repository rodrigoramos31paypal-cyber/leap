  ...
  return (
    <form action={grantPackAction} className="mt-4 space-y-4">
      ...
      <div className="inline-flex flex-wrap items-center gap-1 rounded-lg border border-ink-900/10 bg-white p-1 text-xs dark:bg-ink-800">
        <button type="button" onClick={() => setMode("pack")} className={cn(TAB, mode === "pack" ? TAB_ON : TAB_OFF)}>
          Pack existente
        </button>
        <button type="button" onClick={() => setMode("custom")} className={cn(TAB, mode === "custom" ? TAB_ON : TAB_OFF)}>
          Sessões avulso
        </button>
        <button type="button" onClick={() => setMode("remove")} className={cn(TAB, mode === "remove" ? TAB_ON : TAB_OFF)}>
          Remover sessões
        </button>
      </div>
      ...
      {mode === "custom" && (
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <input name="custom_sessions" type="number" min={1} required defaultValue={1} className="input" />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            ...
            <input name="custom_name" className="input" placeholder="Ex: 2 sessões de Reposição" />
          </div>
        </div>
      )}
      ...
      {mode === "remove" && (
        <div>
          ...
          <p className="mt-1 text-xs text-ink-500">
            Remove sessões do saldo do cliente (consome primeiro as que expiram mais cedo).
            Não afecta a receita.
          </p>
        </div>
      )}
      ...
      {mode !== "remove" && (
        <div className="sm:max-w-xs">
          ...
          <p className="mt-1 text-xs text-ink-500">
            Dinheiro, MBWay e Revolut entram na receita. Cortesia/Oferta atribui as sessões
            sem registar receita.
          </p>
        </div>
      )}

      <button className="btn-primary w-full sm:w-auto">
        {mode === "remove" ? "Remover do cliente" : "Atribuir ao cliente"}
      </button>
    </form>
  );
}
