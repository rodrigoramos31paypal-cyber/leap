...
import { LayoutDashboard, Calendar, Users, CreditCard, BarChart3, Settings, Package, NotebookPen, Megaphone, Store, Images, Trophy } from "lucide-react";
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-[var(--app-height,100dvh)] flex-col overflow-hidden bg-bone-50 dark:bg-ink-900 md:block md:h-auto md:min-h-[100dvh] md:overflow-visible">
      <AppHeight />
      <TopBar
        title="Admin"
        unread={0}
      ...
      <div className="mx-auto flex w-full min-h-0 max-w-7xl flex-1 overflow-y-auto overflow-x-hidden md:flex-none md:overflow-visible md:gap-6 md:px-4 md:py-6">
        <aside className="hidden md:block md:w-56 md:shrink-0 md:sticky md:top-20 md:self-start md:max-h-[calc(100dvh-6rem)] md:overflow-y-auto">
          <nav className="space-y-1">
            ...
            <AdminNavItem href="/admin/leaderboard" icon={<Trophy size={16} />} label="Ranking" />
