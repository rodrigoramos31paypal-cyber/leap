...
// Submete (ou actualiza) a avaliação 1-5⭐ + comentário opcional.
// Idempotente via unique(booking_id) — upsert.
export async function submitRatingAction(formData: FormData) {
  ...
  const { data: b } = await supabase
    .from("bookings")
    .select("id, client_id, trainer_id, status, ends_at")
    .eq("id", bookingId)
  ...
  // Upsert (a unique constraint em booking_id garante 1 por sessão).
